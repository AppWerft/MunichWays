...
var len=_Mathmin(array1.length,array2.length),
lengthDiff=_Mathabs(array1.length-array2.length),
diffs=0,
i;
for(i=0;i<len;i++)(
...
if(!locales[name]&&'undefined'!=typeof module&&
...
function defineLocale(name,values){return(
null===values?(
...
delete locales[name],
null):(values.abbr=name,locales[name]=locales[name]||new Locale,locales[name].set(values),locale_locales__getSetGlobalLocale(name),locales[name]));
...
get_set__get(this,unit):(get_set__set(this,unit,value),utils_hooks__hooks.updateOffset(this,keepTime),this));
...
for('string'==typeof token&&(token=[token]),'number'==typeof callback&&(func=function(input,array){array[callback]=toInt(input)}),i=0;i<token.length;i++)
tokens[token[i]]=func;
...
dayOfMonth=_Mathmin(mom.date(),daysInMonth(mom.year(),value)),
...
get_set__get(this,'Month'):(setMonth(this,value),utils_hooks__hooks.updateOffset(this,!0),this));
...
function warn(msg){
!1===utils_hooks__hooks.suppressDeprecationWarnings&&
...
var
string=config._i,
match=extendedIsoRegex.exec(string)||basicIsoRegex.exec(string),i,l,
...
var matched=aspNetJsonRegex.exec(config._i);return(
...
utils_hooks__hooks.createFromInputFallback(config))):void(config._d=new Date(+matched[1])));
...
week=_Mathfloor((mom.dayOfYear()-weekOffset-1)/7)+1,
resWeek,resYear;
...
var nowValue=new Date(utils_hooks__hooks.now());return(
config._useUTC?
...
var input=[],i,date,currentDate,yearToUse;
...
totalParsedInputLength=0,i,parsedInput,tokens,token,skipped;
...
valid__createInvalid({nullInput:!0}):(
